...
def train_model(request, id):
    ...
    if request.method == 'POST':
        ...
        frame.to_csv('../MachineLearningAPI/static/training/training_'+str(model_ML.back_end_id)+'.csv')
        print(os.listdir('../MachineLearningAPI/static/training'))
        url_train = 'http://m-learning.fr:50/train/'+str(model_ML.back_end_id)
        r_train = requests.post(url_train, headers={'Token':'test_password_12345', "Content-Type":"application/json"}, data=json.dumps({"description":"Test", "training_file":"static/training/training_3.csv", "training_columns":784, "output_column":785, "epochs":1}))
